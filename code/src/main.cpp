...
#include "imageCatalog/PredictionResults.h"
...
//  Rect rect = Rect(dWidth/2.0 - dRectSize/2.0, 1 * dHeight/2.0, dRectSize, dRectSize);
//
...
//
//      PredictionResults results = classifier->getPredictionInfo(resized, mResults);
//
//      cout << "[livePrediction] Classified as " << strClassifiedAs << " " << results.getResultValue()  << endl;
////      imshow("cut", resized);
//
//      mResults.clear();
//    }
...
void livePrediction(CatalogClassificationSVM* classifier)
{
  ...
  double centerX = dWidth / 2.0 - dSize / 2.0;
  double centerY = 1 * dHeight / 2.0;

  PredictionResults best("undefined", 0);
  ...
  while(true)
  {
    ...
    Rect rect = Rect(centerX, centerY, dSize, dSize);
    ...
    if(waitKey(50) == 32)
    {
      ...
      for(int size : sizes) {
        for(int disp : displacements) {
          ...
          PredictionResults results = classifier->getPredictionInfo(resized,
                                                                    mResults);

          if(results.getResultValue() < best.getResultValue()) {
            best.setPredictionResult(results.getPrediction().c_str());
            best.setResultValue(results.getResultValue());
          }
        }
      }

      cout << "[livePrediction] Classified as " << best.getPrediction()
           << " " << best.getResultValue() << endl;

//      imshow("cut", resized);
    }

    stringstream sstm;
    sstm << "You feel " <<  best.getPrediction();
    string strText = sstm.str();
